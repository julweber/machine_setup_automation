#!/usr/bin/env bash
# =============================================================================
# Traefik v3 Reverse Proxy Setup Script
# =============================================================================
#
...
#
# USAGE:
#   ./tasks/setup_traefik.sh                      # Default setup
...
# ┌─────────────────────────────────────────────────────────────────────────┐
# │ COLOURS & HELPERS — standardized output formatting                     │
# │                                                                         │
# │   Color codes:                                                          │
...
# │     step()   → Bold ▶ prefix for major steps                            │
# └─────────────────────────────────────────────────────────────────────────┘
...
# Generated by setup_traefik.sh — do not edit manually
...
# docker-compose.yml — Generated by setup_traefik.sh — do not edit manually
...
if [[ "${TRAEFIK_DASHBOARD}" == "true" ]]; then
  ...
  if [[ "${PASS_GENERATED}" == "true" ]]; then
    echo -e "  ${BOLD}Admin password${RESET}  ${TRAEFIK_ADMIN_PASS}  ${YELLOW}(save this — it will not be shown again)${RESET}"
  else
    echo -e "  ${BOLD}Admin password${RESET}  (as provided via TRAEFIK_ADMIN_PASS)"
  fi
  echo ""
fi
