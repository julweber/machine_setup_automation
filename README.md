...
**Environment variables:** `PROJECT_DIR` (default `/srv/colqwen`), `COLQWEN_MODEL_DIR` (default `~/.cache/huggingface`), `COLQWEN_MODEL` (default `vidore/colqwen2.5-v0.2`), `COLPALI_VERSION` (default `0.3.13`), `NGC_PYTORCH_TAG` (default `25.10-py3`), `COLQWEN_PORT` (default `8100`), `COLQWEN_BIND` (default `127.0.0.1` — the service is unauthenticated, and ufw does not protect Docker-published ports)
