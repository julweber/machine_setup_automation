...
## 2026-08-30T23:45:26+02:00 - worker (ticket 13)

### Implemented: docker group membership fixed — `newgrp` removed, `$USER`-independent idempotent `usermod` (ticket `13-docker-group-membership`)

> Provenance: no ticket file with this name exists in the tree (`docs/tickets/` does not exist; `.tickets/improvements-2/` contains no `13-*.md`; the branch ticket table in `.tickets/improvements-2/README.md` lists number 13 as `13-task-dependency-contract.md`, which is likewise absent from disk and is *not* what this entry records). The criteria below are therefore taken from what actually shipped in commit `31e8dcc` plus the group-membership items of `14-docker-install-post-steps.md` — nothing here is quoted from a ticket-13 document.

- [x] **`tasks/setup-docker.sh` — no interactive shell is ever spawned** — the `newgrp docker >/dev/null 2>&1 || true` fallback (and its `if id -nG | grep -q docker` guard, which read the *current shell's* credentials and so could never see a just-added group) deleted; `grep -rn newgrp tasks/setup-docker.sh` is empty and no stdin-consuming primitive remains in the script. This was the hang: `newgrp` reads the script's own stdin, which under `ssh host ./script.sh`, `run-setup.sh apply` and the VM harness never closes
- [x] **`usermod` no longer depends on `$USER`** — `sudo usermod -aG docker "${USER}"` replaced by `RUN_USER="${USER:-$(id -un)}"` + `sudo usermod -aG docker "${RUN_USER}"`; with `USER` unset (some `sudo -E` / cron invocations) the old line aborted under `set -u` after apt had already modified the system; the fallback resolves the effective uid's name instead
- [x] **membership check is idempotent and queries the group DB** — `id -nG "${RUN_USER}" | tr ' ' '\n' | grep -qx docker` → `User <user> already in docker group` and no `usermod` call on re-runs; the add branch runs `usermod` exactly once
- [x] **the truth is printed instead of a fake success** — the add branch says `Added <user> to docker group - re-login required for unprivileged docker to work`, and the final line was changed from "You may need to log out and back in…" to `Log out and back in (or start a new session) for docker group membership to take effect`; the header `Behaviour:` block and `--help` document "no interactive shell is ever spawned" + the re-login requirement
- [x] **shipped in the same commit (not ticket-13 criteria, recorded for completeness)** — the early exit now requires a *usable* install (`sudo docker compose version` + `sudo docker info`) and the smoke test is the guarded, leak-free `sudo docker run --rm hello-world` that degrades to `warn`; see the `worker (ticket 14)` entry

### Validation
- [x] Static, re-run after `31e8dcc` on the committed tree: `shellcheck tasks/setup-docker.sh` rc=0 (no findings), `bash -n tasks/setup-docker.sh` rc=0, `./tasks/setup-docker.sh --help` rc=0, `grep -rn newgrp tasks/setup-docker.sh` → empty, `grep -n 'usermod -aG docker "${USER}"'` → empty
- [x] **Stub harness** (dev box, PATH override with fake `sudo`/`docker`/`apt`/`apt-get`/`usermod`/`groupadd`/`getent`/`install`/`curl`/`dpkg`, real script + real `lib/helpers.sh`, `timeout` guarded): already-member run prints `User verfeinerer already in docker group` and issues **zero** `usermod` calls; not-member run issues exactly one `usermod -aG docker <user>` plus the re-login hint; `env -u USER` run issues `usermod -aG docker root` and completes rc=0 (no `set -u` abort); missing-group run does `groupadd docker` then one `usermod`
- [x] **No-hang regression**: same script under a pty stdin (`script -qec …`, the condition in which `newgrp` used to swallow the session) returns rc=0 immediately with no interactive prompt and no `# ` shell prompt in the output
- [x] **VM suite (real Ubuntu 26.04 `resolute` VM via virt-runner)** — `tests/run-vm-tests.sh --scripts setup-docker` → `VM TEST SUITE: PASS (3/3 test cases)`; `tests/reports/vmtest-20260830-231146/vmtest/results.jsonl`: precheck rc=0 / 0 s, **integration rc=0 / 24 s**, **idempotency rc=0 / 0 s**; integration log `…/logs/integration-setup-docker.log` contains `Added ubuntu to docker group - re-login required for unprivileged docker to work` and then `[OK] Docker smoke test passed (hello-world).`; the run completed in minutes instead of the earlier `rc=124` at the 1800 s per-script timeout; VM destroyed afterwards, no test VM left defined
- [x] `tests/lint.sh` → `lint guards: PASS` (repo baseline unchanged by this work)

### Notes / assumptions
- **Ticket 13 was delivered together with ticket 14 in commit `31e8dcc`** — both the group-membership fixes and the usable-install/smoke-test fixes landed as one commit against `tasks/setup-docker.sh`; this entry exists so the changelog reflects the group-membership scope separately. No code change accompanies this entry, and `31e8dcc` was neither amended nor rebased.
- **Not verified:** unprivileged docker in a *fresh login session* (`docker ps` as the group-added user after re-login) was only observed **indirectly** — the harness runs both phases inside one non-login ssh session whose probes go through `sudo`, and the VM was destroyed at the end of the run, so no post-run login-session `docker ps` was taken. The group-DB write itself is confirmed (integration log + `usermod` call), the session-visibility claim is standard `systemd-logind` behaviour, not something this run measured.
- **Not verified:** the ticket-14 verification step on a real `docker.io`-only host (`apt-get remove -y docker-compose-plugin && ./tasks/setup-docker.sh`) — covered only by the stub fall-through scenarios; and `--force` was exercised only against the stub harness, not on the VM.
- The interactive `newgrp docker` hints in `README.md:746` and `skills/machine-setup-automation-assistant/SKILL.md:171` remain (operator advice for an interactive shell, where `newgrp` is legitimate); only the script's use of it was removed.

## 2026-08-31T07:15:44+02:00+02:00 - worker (ticket 17)

### Implemented: supply-chain pinning — every image from a pinned `\*_IMAGE` default, download-then-verify installers, reproducible llama.cpp ref (ticket `17-supply-chain-pinning.md`)

- [x] **`lib/helpers.sh` — `warn_moving_image <ref> <var>` + `fetch_and_run <url> <sha-or-empty>` (both `declare -F` guarded)** — the first warns (never fails, never mutates) on `:latest`/`:main`/untagged refs; the second downloads to `mktempfile`, logs `sha256=…`, verifies against an expected digest when one is given, executes the file with `bash` and removes it via a function-`RETURN` trap (plus the existing `mktempfile` exit-tracking chain). Download failure and digest mismatch return non-zero without executing; the installer's own exit status passes through. **Named `warn_moving_image`, not the ticket's `validate_image`**: `tasks/setup-vllm.sh:177` already defines a stricter `validate_image()` (fails on bad characters) that ticket 15 is meant to hoist — a same-named warn-only library function would be shadowed there and would collide with ticket 15's signature
- [x] **concourse** — `: "${CONCOURSE_IMAGE:=concourse/concourse:8.3.0}"` and `: "${POSTGRES_IMAGE:=postgres:15}"` added next to the other defaults, both passed to `warn_moving_image`, both added to the existing explicit `envsubst` lists (direct and Traefik branch); the four `concourse/concourse:latest` / two `postgres:15` literals in `templates/concourse/docker-compose.yml` + `.traefik.yml` became `${CONCOURSE_IMAGE}` / `${POSTGRES_IMAGE}`
- [x] **hermes** — `HERMES_IMAGE` (default `nousresearch/hermes-agent:v2026.8.27`) replaces the old unconditional `HERMES_IMAGE="…:latest"` assignment; the two `sudo cp` render sites now call a new `_render_hermes_compose` which envsubs only `${HERMES_IMAGE}` (`install -m 0644`, temp via `mktempfile`) — `${GITHUB_TOKEN}`/`${OPENROUTER_API_KEY}` stay literal for runtime `.env` resolution. `templates/hermes/docker-compose.yml` differs from HEAD only in the three `image:` lines
- [x] **opencode-server / openwebui** — `OPENCODE_IMAGE` (`ghcr.io/anomalyco/opencode:1.18.25`, GHCR tag carries no leading `v`) added to `load_config` and to the render `export`+`envsubst` list; `OPENWEBUI_IMAGE` (`ghcr.io/open-webui/open-webui:v0.11.1`) added to the defaults, the export list and the envsubst list, replacing `open-webui:main` in both templates — the deliberate `WEBUI_SECRET_KEY` literal-token comment is untouched
- [x] **n8n** — `N8N_IMAGE` (`docker.n8n.io/n8nio/n8n:2.37.4`, the templates were untagged i.e. `:latest`), `N8N_TRAEFIK_IMAGE` (`traefik:v3.7.12`, the sidecar was `image: "traefik"`) and `POSTGRES_IMAGE` (`postgres:15-alpine`, tag unchanged); the render moved from plain `sudo cp` to `_render_n8n_compose` (explicit envsubst list, `install -m 0644`), and the sidecar now carries a comment stating that a second proxy binding 80/443 beside the shared Traefik — with a read-only Docker-socket mount — is a removal decision owned by tickets 19/23. `${DOMAIN_NAME}`/`${SUBDOMAIN}`/`${GENERIC_TIMEZONE}` remain literal and resolve from `${N8N_DIR}/.env` as before
- [x] **planka / omnigent postgres** — `POSTGRES_IMAGE` parameterised with the tag each stack already used (`postgres:16-alpine`; no database minor moved): planka through its existing explicit envsubst lists + export (4 templates), omnigent via Compose's own `${POSTGRES_IMAGE:-postgres:16-alpine}` because that script copies the template verbatim and lets Compose interpolate from `.env`. Documented in both `--help` texts
- [x] **installer downloads** — `tasks/setup-basics.sh` (huggingface-cli, herdr) and `tasks/setup-lm-studio.sh` (llmster) now go through `fetch_and_run` with `HF_CLI_INSTALL_SHA256` / `HERDR_INSTALL_SHA256` / `LMSTUDIO_INSTALL_SHA256` overrides; all three default to empty because no upstream digest for the installer scripts exists (checked 2026-08-31: `hf.co/cli/install.sh.sha256` → 404, the other `.sha256` paths return site HTML), so the digest at least lands in the run log. llmster keeps its previous non-fatal behaviour (`set +e` + `warn`). The nvm line stays pipe-to-shell **with a comment saying why**: its URL is pinned to `v${NVM_VERSION}`, so the bytes are immutable — the property the other three lacked
- [x] **llama.cpp reproducibility** — `: "${LLAMA_CPP_REF:=v0.3.0}"` (latest NON-prerelease at lookup time, 2026-08-25; everything newer is a `b107xx` per-build pre-release published several times a day), ref-shape guard `^[A-Za-z0-9][A-Za-z0-9._/-]*$` so a leading `-` or a shell metacharacter can never reach git; fresh clone uses `git clone --branch "${LLAMA_CPP_REF}"` **without `--depth 1`** (`--depth 1` narrows `remote.origin.fetch` to the single ref, which made any later ref change unfetchable — reproduced in a sandbox), the existing-clone path no longer checks out the remote default branch and `pull --ff-only`es, the `describe --tags --abbrev=0` "latest tag" logic is gone, and both the success line and the final summary print `ref` + `describe` output (`--dirty` included)
- [x] **docs in sync** — README gains an "Image pinning" paragraph and the new env vars for basics, lm-studio, llama-cpp, openwebui, opencode, hermes, planka, concourse, n8n; each touched script's header env-var block and `usage()` list the new variables with their pinned defaults and the lookup date/source, plus an "Update deliberately: …" command
- [x] **`tests/lint.sh` guards** — check 2 widened from `templates/traefik/` to all of `templates/` (the file's own instruction for this ticket), and a new check 3 rejects untagged `image:` pulls with the nanobot stack allowlisted (those two images come from `build:`, they are not pulls)

### Validation
- [x] **Ticket acceptance greps** — `grep -rnE "image: .*(:latest|:main)$" templates/ | grep -v socket-proxy` → empty; `grep -rn '^\s*image: "traefik"$' templates/` → empty; `grep -rn "curl .*| *bash" tasks/` → exactly one line (`tasks/setup-basics.sh:158`, nvm); `fetch_and_run` present at the three intended call sites; `grep -n -- '--branch ""' tasks/setup-llama-cpp.sh` → line 336 and `--help` lists `LLAMA_CPP_REF`
- [x] **Linters** — `shellcheck -x` + `bash -n` clean on all 12 modified `.sh` files; `yamllint -c templates/.yamllint templates/` → no errors (only the repo's pre-existing `document-start` warnings); `tests/lint.sh` → `lint guards: PASS` with all three guards green
- [x] **Render + Compose semantics** — all 13 changed template variants rendered with each script's real envsubst variable list: 26/26 assertions (image line substituted, no moving/untagged image after render), every remaining `${…}` is a runtime/`.env` value (`WEBUI_SECRET_KEY`, `DATABASE_URL`, `SECRET_KEY`, `POSTGRES_PASSWORD`, `CONCOURSE_*`, `GITHUB_TOKEN`, `OPENROUTER_API_KEY`, …), and `docker compose config -q` accepts all 13 rendered files
- [x] **Render helpers executed** — `_render_hermes_compose` and `_render_n8n_compose` extracted from the scripts and run against a temp destination (fake `sudo`): mode 0644, pinned images, runtime variables literal, no temp file left behind
- [x] **`fetch_and_run` behaviour** — matching digest → executes (`installer ran`, rc=0); mismatching digest → `checksum mismatch … refusing to execute`, rc=1; unreachable URL → `download failed`, rc=1; empty expected digest → digest logged and executed; no installer temp file survives the call
- [x] **llama.cpp fetch/checkout flow** — 7 scenarios against a local `file://` remote (fresh clone at tag, re-run same ref, switch to older tag, switch to branch, re-run branch, back to tag, unknown ref rejected with the actionable error) plus the ref-shape guard: rejects `-b`, `v1;id`, `../x`, `--upload-pack=…` and empty, accepts `v0.3.0`/`master`/`feature-x`/`release/v1.2`, no injection artifact created
- [x] **`--help` smoke** — basics, hermes, n8n, concourse, openwebui, planka, llama-cpp, lm-studio, omnigent all exit 0 with no `set -u` or command-not-found errors (an early `validate_image`-before-default ordering bug in `setup-hermes.sh` was caught this way and fixed); `setup-opencode-server.sh` handled by inspection (`main()` returns on `--help` before any action)
- [x] **Not run in this environment** — the VM/integration suite (`tests/run-vm-tests.sh`) and a real `setup-llama-cpp.sh` compile: no disposable host here. Pinning correctness is therefore proven statically, by rendering and by the sandbox git flow, not by pulling the pinned images or building

### Notes / assumptions
- Pinning a default **is** a version decision: a re-run against an install that had pulled a newer `latest`/`main` (openwebui `main` → `v0.11.1`, opencode/hermes/n8n/concourse likewise) now converges it onto the recorded tag. That is what the ticket asks for; per-stack tags were chosen as the current stable release and every one carries its lookup date and source URL so the next bump is a deliberate edit
- No postgres minor moved (`15`, `15-alpine`, `16-alpine` stay as they were) — database upgrades are not part of this ticket's scope and a silent minor bump against an existing volume is exactly the risk ticket 12's converge policy is meant to avoid
- Ticket dependency `15-lib-validation-helpers.md` has not landed; only the moving-tag warning this ticket needed was added (`warn_moving_image`), none of ticket 15's broader validator set. Moving these call sites onto ticket 15's `validate_image` later is a one-word change per site
- **Moving defaults left in place outside this ticket's file inventory** (changing an application's default version is an upgrade/downgrade decision, not a template fix): `PLANKA_IMAGE=…planka:latest` (`tasks/setup-planka.sh:141`), `OMNIGENT_IMAGE_TAG=latest`, `EXCALIDRAW_IMAGE=excalidraw/excalidraw:latest` (upstream publishes only `latest` + `sha-*` on Docker Hub), `NEXTCLOUD_IMAGE=nextcloud:stable` plus an inline `mariadb:lts`/`redis:alpine` heredoc, `TRAEFIK_IMAGE=traefik:v3`, `LLAMA_SWAP_VERSION=latest`, `VLLM_OMNI_VERSION=latest`. A follow-up ticket with a per-service bump + migration check is recommended
- Untouched as the ticket instructs: the four `templates/traefik/docker-compose.socket.*.yml` socket-proxy files (ticket 07), the n8n sidecar's *existence*, `POSTGRES_PASSWORD=changeme` and `.env` mode hygiene (ticket 23), `tasks/setup-vllm.sh`'s own `validate_image`
